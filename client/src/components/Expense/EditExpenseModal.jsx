...
const EditExpenseModal = ({open, onClose}) => {
    ...
    const {setExpenses, Expenses} =useExpenseTracker()
    ...
    const handleSubmit =async (e) => {
        const ExpenseData ={source, amount, date:new Date(date)}
        e.preventDefault();
      
        setError('')
        setLoading(true)
        try {
           const res =await axiosInstance.put(`/api/Expenses/${id}`, ExpenseData)
           setExpenses(Expenses.map((item)=> item._id === id ? res.data : item))
           navigate('/Expense')
        } catch (error) {
            console.log(error)
        }finally{
